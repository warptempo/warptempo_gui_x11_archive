...
int main(int argc, char* argv[]) {
    ...
    if (!timemap.empty()) out_sfinfo.frames = (long)timemap.back().target_frame;
    ...
    long total_target_frames = (long)timemap.back().target_frame;
    if (total_target_frames <= 0) total_target_frames = sfinfo.frames;

    std::cout << "Processing..." << std::endl;

    while (current_output_frames_generated < total_target_frames) {
