...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        std::cerr << "Usage: " << argv[0] << " <source_audio> <timemap_file> <target_audio> [N=3328] [low_thresh_hz=50.0]\n";
        return 1;
    }
    ...
    if (!src_snd) { 
        ...
        std::cerr << "Libsndfile reason: " << sf_strerror(NULL) << "\n";
        return 1; 
    }
    ...
    // 3. FFTW Setup (Strictly FFTW_ESTIMATE for Stage 1 & 3 determinism)
    ...
    // ========================================================================
    // STAGE 1: The Dry Pass (Virtual Output)
    // ========================================================================
    std::cout << "[Stage 1] Executing Dry Pass (N=" << N << ") -> Virtual Memory Buffer...\n";
    ...
    size_t t_s = 0; 
    int frame_idx = 0;
    int frames_to_skip = N / 2; // Track latency trim
    ...
    while (t_s < target_total_frames) {
        ...
        for (int ch = 0; ch < channels; ++ch) {
            ...
            for (int n = 0; n < N; ++n) {
                overlap_add[ch][n] += (ifft_out[n] / N) * synth_window[n];
            }
        } // End of channel loop
        ...
        for (int n = write_offset; n < write_offset + write_len; ++n) {
            for (int ch = 0; ch < channels; ++ch) {
                virtual_tgt_buf.push_back(static_cast<float>(overlap_add[ch][n]));
            }
        }
    ...
    // ========================================================================
    // STAGE 2: Integrated EQMatch Diagnostics
    // ========================================================================
    std::cout << "\n[Stage 2] Profiling Source Acoustics & Virtual PSD Delta...\n";
    ebur128_state* st = ebur128_init(channels, src_info.samplerate, EBUR128_MODE_M | EBUR128_MODE_I);
    
    // 2A. Profile Source LUFS
    ...
    // ========================================================================
    // STAGE 3: The Wet Pass & Single-Pass Synthesis
    // ========================================================================
    std::cout << "\n[Stage 3] Executing Wet Pass with Native EQ Synthesis...\n";
    ...
    if (!tgt_snd) {
        std::cerr << "Error: Could not open Target file for writing. " << sf_strerror(NULL) << "\n";
        return 1;
    }
    ...
    while (t_s < target_total_frames) {
        ...
        for (int ch = 0; ch < channels; ++ch) {
            ...
            for (int k = 0; k <= N / 2; ++k) {
                phi_prev[ch][k] = phi[k]; theta_prev[ch][k] = theta[k];
                M[k] *= multiplier_array[k]; // Apply Target EQ natively
                ifft_in[k][0] = M[k] * std::cos(theta[k]);
                ifft_in[k][1] = M[k] * std::sin(theta[k]);
            }
        ...
        if (write_len > 0) {
            for (int n = 0; n < write_len; ++n) {
                for (int ch = 0; ch < channels; ++ch) {
                    write_buf[n * channels + ch] = static_cast<float>(overlap_add[ch][write_offset + n]);
                }
            }
    ...
    sf_close(src_snd); sf_close(tgt_snd);
